...
let display (vertexBuffer, colourBuffer) = function () ->
  ...
  glEnableClientState GL_COLOR_ARRAY;           (* --- VBO's in action. *)
    ...
    begin
      ...
      let mapped_buffer = glMapBufferAbs GL_ARRAY_BUFFER GL_WRITE_ONLY in
      (* mapped_buffer_blit mapped_buffer vertexData   (* update all the buffer *)
                            ((Bigarray.Array1.dim vertexData) * (elem_size vertexData)); *)
      mapped_buffer_blit_ofs mapped_buffer vertexData  (* update a piece of the buffer *)
                             ~ofs:0 ~len:(1 * (elem_size vertexData));
